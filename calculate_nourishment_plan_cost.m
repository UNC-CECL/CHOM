...
% function [BPC]=calculate_nourishment_plan_cost(ACOM,M,MMT,X_NOF,X_OF,X_OF_msd)
% 
% % note using a 30 year interval over which to calculate expected beach width could
% % increase nourishment frequency, since a 30 year time frame will push down the 
% % the no-nourishment expected beach width (incorporating more 0-width years)
% 
% t = M.time;
% 
% I_msd=ACOM.I_msd;
...
%     namount            = nourish_xshore*Ddepth*Llength*sandcost;
%     varcost            = namount./((1+delta).^nourish_yr);
%     maxplan            = find(nourish_yr>10);  % how many years to consider costs over?
%     maxplan            = maxplan(1)-1;
%     BPC.cost(i)        = sum(fcost(1:maxplan))+sum(varcost(1:maxplan));
%     
%     if t>30
%         bw_future = [MMT.bw(1:t); bw]; % consists of past beach width plus future expected nourishments
%         ind = 1;
...
%     
%     % get base tax rate
%     BPC.tau_add(i) = (MMT.amort)*BPC.tc_peryear(i)/sum(MMT.taxratio_OF*I_OF.*(1-I_msd)*X_OF.price(t-1) + ...
%         (1-I_OF)*X_NOF.price(t-1) + MMT.taxratio_msd*I_OF.*I_msd*X_OF_msd.price(t-1));
%     
%     % total tax burden
%     BPC.tax_burden(:,i) = (MMT.amort)*(BPC.tau_add(i)*(1-I_OF)*X_NOF.price(t-1) + ...
%         MMT.taxratio_OF*BPC.tau_add(i)*I_OF.*(1-I_msd)*X_OF.price(t-1) + ...
%         MMT.taxratio_msd*BPC.tau_add(i)*I_OF.*I_msd*X_OF_msd.price(t-1));
%     
% end
% 
% 
